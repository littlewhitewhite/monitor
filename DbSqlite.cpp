...
CSqlStatement::Bind(int pos_zero_indexed, LPCTSTR value)
{
   ...
   int rc = _sqlite3_bind_text(m_stmt, pos_zero_indexed+1,
                             v, v.GetLength() * 2, SQLITE_TRANSIENT);
...
CSqlStatement::Execute()
{
   ...
   if ( rc == SQLITE_BUSY )
   {
      szMessage = _T("SQLITE_BUSYtt1111");
...
CSqlStatement::NextRow()
{
   ...
   {
      szError = _T("CSqlStatement::NextRow SQLITE_BUSYtttttt");
   }
...
bool CDbSQLite::SetBusyTimeOut(int ims)
{
	bool fResult = TRUE;
	int rc = _sqlite3_busy_timeout(m_db, ims);
	if (rc != SQLITE_OK)
	{
		fResult = false;
	}
